...
function noStockTags() {
  const f = SALE.form;
  if (!f || f.kind !== 'online') return [];
  return SALES.NOSTOCK.filter(o => f.noStock[o.key]).map(o => o.tag);
}
...
function srcNote() {
  const c = saleSrc();
  if (skipStock()) {
    return `<div class="hint-row nostock">🚫 這張單<b>不扣庫存</b>（${sEsc(noStockTags().join('、'))}），只記錄銷售與營收</div>`;
  }
...
function noStockBlock() {
  const f = SALE.form;
  return `<div class="field nostock-box">
    ${SALES.NOSTOCK.map(o => `<label class="check-inline${f.noStock[o.key] ? ' on' : ''}">
        <input type="checkbox" data-nostock="${o.key}" ${f.noStock[o.key] ? 'checked' : ''}>${sEsc(o.label)}
      </label>`).join('')}
  </div>`;
}
...
function renderSaleBody() {
  ...
  if (f.kind === 'online') {
    html += `<div class="field"><label>客戶名稱 <span class="req">*</span></label>
      ...
      + itemsBlock() + srcNote() + noStockBlock()
...
function wireSaleBody() {
  ...
  document.querySelectorAll('[data-nostock]').forEach(el => el.onchange = e => {
    f.noStock[el.dataset.nostock] = e.target.checked;
    renderSaleBody();
  });
